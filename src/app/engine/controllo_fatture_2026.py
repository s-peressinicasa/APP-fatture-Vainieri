...
TARIFFARIO_PATH = BASE_DIR / "prezzi_vainieri_2026.xlsx"
...
# ==============================
# 3) CODIFICA TARIFFARIO (2026) + SCONTI
# ==============================
...
def _is_all_country(val) -> bool:
    v = _norm_str(val)
    return ("TUTTO IL PAESE" in v) or ("TUTTOILPAESE" in v) or (v == "ALL") or ("ALL COUNTRY" in v)
...
def _norm_country_from_sheet(sheet_name: str) -> str:
    s = _norm_str(sheet_name)
    if s in {"FRANCIA", "FR"}:
        return "FR"
    if s in {"UK", "GB"}:
        return "UK"
...
class TariffCountry:
    def __init__(self):
        self.code_to_zone: Dict[str, str] = {}
        self.rate_by_zone: Dict[str, float] = {}
        self.default_rate: Optional[float] = None

    def zone_for_code(self, code: str) -> Optional[str]:
        code_u = _norm_str(code)
        if code_u.isdigit() and len(code_u) == 1:
            code_u = code_u.zfill(2)
        z = self.code_to_zone.get(code_u)
        if z:
            return z
    ...
    def base_rate(self, zone: Optional[str]) -> Optional[float]:
        if zone is None:
            return self.default_rate
        z = _norm_str(zone)
        if z in self.rate_by_zone:
            return float(self.rate_by_zone[z])
        if self.default_rate is not None:
            return float(self.default_rate)
        return None

def _pick_base_rate_from_row(row: pd.Series, cols: List[str], postal_col: Optional[str], zone_col: Optional[str]) -> Optional[float]:
    """Estrae la tariffa base €/mc da una riga tariffario.

    - Preferisce colonne esplicite come '€/mc' o 'Tariffa'
    - Fallback: se ci sono più colonne numeriche (es. fogli legacy con colonne per fascia volume),
      assume che la tariffa *base* sia il valore più alto tra le colonne tariffarie disponibili.
    """
    rate_col = _find_col(
        cols,
        [
    ...
    if rate_col and pd.notna(row.get(rate_col)):
        try:
            return float(row.get(rate_col))
        except Exception:
            return None

    candidate_cols = [c for c in cols if c not in {postal_col, zone_col}]

    numeric_vals: list[tuple[str, float, str]] = []
    for c in candidate_cols:
        ...
        numeric_vals.append((c, f, header))

    if not numeric_vals:
        return None

    # colonne che "sembrano" tariffarie (€/mc, tariffa, mc, m3, ecc.)
    tariff_like = [
        (c, f, h)
        for (c, f, h) in numeric_vals
        if any(tok in h for tok in ["tariff", "€/","eur", "euro", "mc", "m^3", "m3", "m³"])
    ]

    if tariff_like:
        _, best, _ = max(tariff_like, key=lambda t: t[1])
        return float(best)

    # ultimo fallback: prendi il massimo numerico disponibile
    _, best, _ = max(numeric_vals, key=lambda t: t[1])
    return float(best)


def load_tariffario(path: Union[str, Path]) -> Dict[str, TariffCountry]:
    xls = pd.ExcelFile(path)
    out: Dict[str, TariffCountry] = {}
    for sh in xls.sheet_names:
        country = _norm_country_from_sheet(sh)
        df = pd.read_excel(path, sheet_name=sh, engine="openpyxl")

        postal_col = _find_col(df.columns.tolist(), ["codici postali", "Codici Postali", "postal", "zip"])
        zone_col = _find_col(df.columns.tolist(), ["Zona", "zona"])
        ...
        for _, row in df.iterrows():
            # zona
            zone = _norm_str(row.get(zone_col)) if zone_col else "ALL"
            if not zone:
                zone = "ALL"

            # tariffa base
            rate = _pick_base_rate_from_row(row, cols, postal_col, zone_col)
            if rate is None:
                continue

            # default ("Tutto il Paese")
            if postal_col and _is_all_country(row.get(postal_col)):
                tc.default_rate = float(rate)
                tc.rate_by_zone["ALL"] = float(rate)
                continue

            tc.rate_by_zone[zone] = float(rate)

            # mapping codici postali -> zona
            if postal_col:
                for code in _parse_codes(row.get(postal_col)):
                    # normalizzazione digit 1 -> 01
                    if code.isdigit() and len(code) == 1:
                        code = code.zfill(2)
                    tc.code_to_zone.setdefault(code, zone)

        out[country] = tc
    return out

TARIFFARIO = load_tariffario(TARIFFARIO_PATH)

SPECIAL_CLIENT_NAME = "ERCOL FURNITURE LIMITED"
SPECIAL_CLIENT_ALIASES = {"ERCOL", "ERCOL FURNITURE LIMITED"}
SPECIAL_SCARICO_CONTAINS = "F. EDMONDSON & SONS"
SPECIAL_RATE_OVER_15 = 121.5  # €/mc
def _contains_edmondson(text: Optional[str]) -> bool:
    """Match tollerante per 'F. EDMONDSON & SONS' nello scarico (gestisce punteggiatura/varianti)."""
    if text is None:
        return False
    s = _norm_str(text)
    s = re.sub(r"[^A-Z0-9& ]+", " ", s)
    s = re.sub(r"\s+", " ", s).strip()
    return ("EDMONDSON" in s) and ("SONS" in s)


def _discount_pct(volume_bill: float, country: str, zone: Optional[str]) -> float:
    """Sconti non cumulativi, validi per tutte le zone tranne Corsica e Irlanda."""
    try:
        v = float(volume_bill)
    except Exception:
        return 0.0
...
def expected_rate_per_m3(country: str, zone: Optional[str], volume_bill: float, cliente: Optional[str] = None, scarico: Optional[str] = None) -> Optional[float]:
    """Tariffa attesa €/mc, includendo sconti e regola speciale cliente."""
    c = _norm_str(country)
    z = _norm_str(zone)
    cli = _norm_str(cliente)

    # Regola speciale: cliente ERCOL + scarico contiene "F. EDMONDSON & SONS".
    # Applica solo sopra 15mc: 121.5 €/mc.
    sca = _norm_str(scarico)
    # Nota: il campo "Cliente" non è sempre disponibile (es. molte spedizioni UK non hanno Cliente valorizzato).
    # In questi casi, se lo scarico contiene "F. EDMONDSON & SONS" consideriamo comunque la tariffa ERCOL.
    if _contains_edmondson(scarico) and c == "UK":
        try:
            if float(volume_bill) > 15:
                if ("ERCOL" in cli) or (cli in {_norm_str(x) for x in SPECIAL_CLIENT_ALIASES}) or (cli == ""):
    ...
    disc = _discount_pct(volume_bill, c, z)
    return float(base) * (1.0 - float(disc))

def get_destination_info(scarico: str):
    """
    Ricava:
      - country: 'FR','UK','DE','BE','IE','CH'
      - dest_code: codice in parentesi (es. '33', '2A', 'AL', ecc.) se presente
      - zone: zona tariffaria (o 'ALL' se tariffa "Tutto il Paese")
    dall'indirizzo di scarico.

    Per i paesi con tariffa unica (es. BE/CH/IE) è sufficiente la sigla paese dopo il trattino
    (es. "... - BE") anche senza codice tra parentesi.
    """
    if scarico is None or (isinstance(scarico, float) and pd.isna(scarico)):
        return None, None, None

    s = str(scarico).replace("\u2019", "'")
    su = s.upper()

    # Caso standard: "(XX) - FR"
    m = re.search(r"\(([^)]+)\)\s*-\s*([A-Z]{2})\b", su)
    if m:
        code = _norm_str(m.group(1))
        country = _norm_str(m.group(2))
        if country == "GB":
            country = "UK"

        # Irlanda del Nord: se la spedizione risulta UK ma il codice in parentesi inizia con "BT",
        # va trattata come Irlanda (IE) e applicata la tariffa "Tutto il Paese".
        if country == "UK" and str(code).strip().upper().startswith("BT"):
            country = "IE"
            code = ""
            tc_ie = TARIFFARIO.get(country)
            if tc_ie and tc_ie.default_rate is not None:
                return country, code, "ALL"
            # se manca IE nel tariffario, segnalo tariffa mancante
            return country, code, None

        tc = TARIFFARIO.get(country)
        if not tc:
            return country, code, None

        zone = tc.zone_for_code(code)
        return country, code, zone

    # Fallback: nessun codice tra parentesi. Se il paese ha tariffa unica ("Tutto il Paese"),
    # basta riconoscere la sigla paese dopo il trattino.
    countries = re.findall(r"-\s*([A-Z]{2})\b", su)
    if not countries:
        return None, None, None

    country = _norm_str(countries[-1])
    if country == "GB":
        country = "UK"
    ...
    if tc.default_rate is None:
        # per FR/UK/DE ecc. serve il codice per determinare la zona
        return None, None, None

    # Paese con tariffa unica
    return country, "", "ALL"
...
def check_invoice(df_ship: pd.DataFrame, tolerance: float = 0.01):
    ...
    for gid, grp in df_ship.groupby("group_id"):
        ...
        country = grp.iloc[0].get("country")
        dest_code = grp.iloc[0].get("dest_code")
        zone = grp.iloc[0].get("zone")

        # Cliente (se disponibile)
        cliente = ""
        if "cliente" in grp.columns:
            vals = [v for v in grp["cliente"].tolist() if isinstance(v, str) and v.strip()]
            cliente = vals[0] if vals else ""

        # Scarico (se disponibile)
        scarico = ""
        if "scarico" in grp.columns:
            vals_s = [v for v in grp["scarico"].tolist() if isinstance(v, str) and v.strip()]
            scarico = vals_s[0] if vals_s else ""
        ...
        # -------------------------
        if len(grp) > 1:
            vol_total_round = 1.0 if vol_total_raw < 1 else _ceil_01(vol_total_raw)

            tariff = expected_rate_per_m3(country, zone, vol_total_round, cliente=cliente, scarico=scarico)
        ...
        # -------------------------
            ...
            if vol > 1.0:
                ...
                tariff = expected_rate_per_m3(country, zone, vol_arrotondato, cliente=cliente, scarico=scarico)
                if tariff is None:
                    e = base_info.copy()
                    e.update(
                        {
            ...
            # 2) volume <= 1 m3
                ...
                tariff = expected_rate_per_m3(country, zone, 1.0, cliente=cliente, scarico=row.get("scarico") if isinstance(row, dict) else None)
...
def crea_report_excel(
    ...
    # ==========================
    # Preparazione colonne tecniche (DDT6) + caricamento excel Francia (se presente)
    # ==========================
...
# 6B) COLONNE ERRORI + EXPORT
    ...
    with pd.ExcelWriter(report_path, engine="xlsxwriter") as writer:
        ...
        _set_col_width("Qta", 6)
        _set_col_width("Iva", 5)
        _set_col_width("Importo", 12)
        _set_col_width("Note", 30)
        _set_col_width("Errori", 55)
        _set_col_width("Errori Confronto Volume Francia", 55)
